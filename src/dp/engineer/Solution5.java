...
//				System.out.println("mmmm : " + max);
	...
	private static void solve(int status, int empIdx, int unitIdx, boolean[] visit) {
		if(unitIdx > N) {
			return;
		}
		for (int i = 0; i < K; i++) {
			if(!visit[i] || empIdx == i) {
				...
				status = change(visit);
				for(int j = 1; j <= N; j++) {
						dp[status][j] = Math.max(dp[status][j - 1] + worker[i][j], dp[status][j]);
						System.out.println(" dp["+status+"]["+j+"] : " + dp[status][j]);
					if(j == N) {
						max = Math.max(max, dp[status][j]);
					}
				}
			}	
		}
